...
`CompletionSummarySection` コンポーネント SHALL 契約コース情報と保険料合計を表示する。表示内容は Zustand ストアの `contractCourse` データに基づく。`CompletionSummarySection` SHALL ストアの `insurancePremium` 値を保険料合計として表示する。ストアの `insurancePremium` 初期値は 880 とする。
