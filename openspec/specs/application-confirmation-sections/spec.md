...
各確認セクション SHALL マゼンタ（--color-primary）背景の見出しとクリーム（--color-cream）背景のコンテンツエリアで構成されること。ラベルと値は1行のペア構成で表示すること。ラベルの幅は申込入力画面の項目ラベル幅（260px）と同一とし、値の幅は申込入力画面の項目値幅（1fr）と同一とすること。レイアウトには申込入力画面のFormRowコンポーネントと同じgrid-cols-[260px_1fr]を使用すること。
...
#### Scenario: ラベル幅が申込入力画面と同一であること
- **WHEN** 任意の確認セクションがレンダリングされる
- **THEN** ラベルの幅が260px（grid-cols-[260px_1fr]の第1カラム）であること

#### Scenario: 値幅が申込入力画面と同一であること
- **WHEN** 任意の確認セクションがレンダリングされる
- **THEN** 値の幅が1fr（grid-cols-[260px_1fr]の第2カラム）であること
